...
#    # XGBClassifier XGBRegressor
    ...
    arg_parser.add_argument('--weights', type=str, help="预训练权重文件地址", default='weights/yolov5m.pt')

    args = arg_parser.parse_args()
    logging.info("{} args: {}".format(__file__, args))
    data_config = open('/yolov5/data/data.yaml').read()
    train_config = open('/yolov5/models/yolov5m.yaml').read()
    ...
    with open('/yolov5/models/yolov5m.yaml','w') as f_train_cfg:
        f_train_cfg.write(train_config)
    with open('/yolov5/data/data.yam','w') as f_data_cfg:
